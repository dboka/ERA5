...
ROOT_OUT="/home/denissbokadenissboka/projects/gridpp_lab/gridpp_precip"
...
# --- GridPP config (requested) ---
LVAL="40000"
POBS_D="0"
MAX_POINTS="29"
CV_RADIUS="0"

# try a few eps values (log-ratio can be sensitive)
EPS_LIST=("0.1" "1" "5")

for EPS in "${EPS_LIST[@]}"; do
  OUT_BASE="${ROOT_OUT}/out_${YEAR}_html_compare_eps${EPS}_L${LVAL}_p${POBS_D}_r${CV_RADIUS}_ukR${UK_RANGE_M}"
  mkdir -p "$OUT_BASE"
  ...
  for m in $(seq -w 1 12); do
    ...
    "$VENV_PY" "$REPORT_PY" \
      ...
      --cv_radius "$CV_RADIUS" \
      --title "LOOCV ${YEAR}-${m}: GridPP vs UK (eps=${EPS}, L=${LVAL}, pobs=${POBS_D}, r=${CV_RADIUS})"
...
out_html="$INDEX_HTML"
df=pd.read_csv(summary_path).sort_values(["year","month"])

def row(r):
    m=int(r["month"]); mm=f"{m:02d}"
    link=f"{r['out_dir']}/report.html".replace("$OUT_BASE/","")
    return (
      "<tr>"
      f"<td>{r['year']}-{mm}</td>"
...
<body>
<h1>LOOCV salīdzinājums {df['year'].iloc[0]} (eps={df['year'].iloc[0] and ""})</h1>
<div class="small">
GridPP log-ratio OI vs UK. Parametri: eps={"{EPS}"}; L={"{LVAL}"}; pobs_d={"{POBS_D}"}; cv_radius={"{CV_RADIUS}"}; UK range={"{UK_RANGE_M}"}.
</div>
<div style="height:14px"></div>
<table>
<thead>
<tr>
  <th>Mēnesis</th><th>N</th>
